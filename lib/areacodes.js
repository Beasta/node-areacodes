...
class AreaCodes {
	...
	getByAreaCode( areaCode, done ) {

		if ( typeof areaCode === 'number' ) {
			areaCode = areaCode.toString();
		} else if ( typeof areaCode !== 'string' ) {
			return done( new Error( 'Invalid area code: incorrect type' ) );
		}

		if ( areaCode.length !== 3) {
			return done(new Error('Invalid area code: not 3 digits in length'));
		}

		areaCode = areaCode.match( /^([0-9]{3})$/ );
